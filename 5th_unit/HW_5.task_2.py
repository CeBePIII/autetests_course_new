...
def repeats(our_str):
    """
    Функция добавляет после каждого символа в строке постфикс _n, где n - число повторений данного символа
    на текущей позиции
    ...
    for key in our_str:
        if key in letter_dict:
            letter_dict.update({key: letter_dict[key] + 1})
        else:
            letter_dict[key] = 1
        new_str = f"{new_str}{key}_{letter_dict[key]}"
    return new_str
